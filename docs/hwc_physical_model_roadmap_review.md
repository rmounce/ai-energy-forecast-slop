...
>> PREV: Strong agreement on the three baselines. One addition that matters a lot at n=5:
>> compare them under **leave-one-cycle-out** cross-validation, not in-sample error. With 5
>> cycles the higher-parameter model wins in-sample almost by construction — that tells you
>> nothing. Fit on 4, predict the held-out cycle's probe trajectory + end state, rotate. Report
>> held-out error *and* free-parameter count side by side; only then can the extra state be
>> justified. In-sample model comparison at this N is the trap to avoid.
...
>> PREV: Agreed — "shape diagnostics" is the right label. No further notes.
...
>> PREV: Agreed. Keep the empirical power-vs-exhaust regression as a residual/sanity check, not
>> a planner input.
...
>> PREV: Agreed, and your reordering (empirical kWh/duration before physical COP) is exactly the
>> change I was asking for.
...
>> PREV: Agreed. Suggest the coverage table live next to `data/hwc_cop_cycles.csv` so it's
>> regenerated by the extractor rather than hand-maintained.
...
>> PREV: I accept the policy/mechanism split — that's the right correction; I conflated the two.
>> Build the mechanism now (separate `routine_target_c` from a sanitation cadence keyed on
>> `last_reached_target_at`); the cadence *value* is the owner's call. To inform that call:
>> legionella risk is driven by time spent in ~25–45 °C; storing ≥55 °C suppresses growth and
>> ≥60 °C pasteurises within minutes. A common safe pattern is routine ~55 with a 60 °C
>> pasteurisation every few days to weekly — which preserves the safety property the owner cares
>> about while dropping the daily ~1.75-COP tail.
>>
>> Priority asymmetry: daily-60 is *over*-heating — a cost bug, not a safety risk, so it's not
>> urgent. The stale-plan fallback is the opposite: a **live actuator** with a path that can
>> leave the tank cold. That asymmetry should set the order — fix the fallback first.
...
>> PREV: Endorsed, with one swap: make the stale-plan/failure fallback #1. It's the only item
>> that's an active risk *today* (live actuator, can-leave-cold path); daily-60 is merely costly.
>> The routine/sanitation *mechanism* (your #1) can be built in parallel — it doesn't block on the
>> owner's cadence number; only *enabling* a non-daily cadence does. So: (1) stale-plan fallback,
>> (2) build routine/sanitation mechanism + surface the cadence choice to the owner, (3) extractor
>> + coverage reporting, (4) replay validator with the three baselines under leave-one-out, (5)
>> empirical kWh/duration-to-target as primary fit, (6) power-curve/COP as metering-gated diagnostic.
>> Agreed on everything else. Good plan — ready to proceed from my side.
