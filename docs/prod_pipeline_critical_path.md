...
**Out of scope** (deliberately): the archived shadow forecast surfaces
(PD-direct, canonical AI MPC/DH bundle, AEMO stitched, TFT-load, p5min
...
- **The archived AI MPC/DH bundle sensors** (`sensor.ai_mpc_*_price_forecast`,
  `sensor.ai_dh_*_price_forecast`, `sensor.ai_pd_direct_*`,
  `sensor.ai_aemo_price_forecast`). These were disabled on 2026-06-15 and
  should not be expected to update. EMHASS reads raw Amber for MPC and
  `sensor.ai_price_forecast` for DH.
- **TFT-load shadow**. Disabled on 2026-06-15; EMHASS consumes the LightGBM load
  forecast.
- **AEMO ingest timers** (`ai-energy-p5min`, `ai-energy-predispatch`,
  `ai-energy-pd7day`, `ai-energy-sevendayoutlook`). Still active for data
  collection. They no longer chain tactical or PD-direct shadow publishers.
- **`forecast.py publish-tactical`** (chained to p5min ingest) and
  **`publish-pd-direct`** (chained to predispatch ingest). Both are now no-op
  archived commands.
