...
| Running compressor policy | planner scores stop/continue candidates; temporary experiment uses `stop_cost_aud = $0.05` |
| Temporary short-cycle experiment | **active 2026-06-20**: `thermal.min_temp = 50`, restore to `45`; `stop_cost_aud = 0.05` |
...
- **Temporary short-cycle experiment (2026-06-20):** `thermal.min_temp = 50` and
  `block_planner.stop_cost_aud = 0.05` to force likely top-up chances and observe Aquatech
  start/stop behaviour. Restore to `min_temp = 45` after review.
