#!/usr/bin/env python3
"""
Train a TFT-style price forecast model for SA1 using AEMO PREDISPATCH data.
...
Architecture: LSTM encoder-decoder with cross-attention (simplified TFT).
  ...
  Decoder (32 steps = 16h):    PREDISPATCH forecasts + cyclic time encodings
  Output:                      3-quantile prediction [q10, q50, q90] in $/MWh

Covariate construction: Option B (run-aligned) — each training sample uses the
PREDISPATCH run issued at the encoder/decoder boundary. Matches inference exactly.
See data/build_training_dataset.py for construction details.
...
Input data: pre-built numpy arrays from data/build_training_dataset.py
  ...
  data/parquet/X_decoder.npy     [N, 32, 9]   — normalised decoder features
  data/parquet/y_targets.npy     [N, 32]      — normalised target RRP
  data/parquet/y_targets_raw.npy [N, 32]      — raw target RRP (evaluation)
...
Usage:
  python train/train_tft_price.py             # train with defaults
  python train/train_tft_price.py --epochs 50 --batch-size 128 --d-model 128
...
class QuantileLoss(nn.Module):
    ...
    def forward(self, preds: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        targets = targets.unsqueeze(-1).expand_as(preds)      # [B, T, Q]
        for i, q in enumerate(self.quantiles):
            e = targets[:, :, i] - preds[:, :, i]
            loss = loss + torch.mean(torch.max(q * e, (q - 1) * e))
...
def evaluate_nmape(model, loader, scalers):
    """Compute nMAPE on raw (denormalised) predictions vs raw targets."""
    model.eval()
    ...
    with torch.no_grad():
        for X_enc, X_dec, y_norm in loader:
    ...
    # nMAPE = mean(|pred - actual| / max(|actual|, 1)) × 100
    denom = np.maximum(np.abs(targ), 1.0)
    nmape = np.mean(np.abs(pred - targ) / denom) * 100
    return nmape
...
def main():
    ...
    parser.add_argument("--d-model", type=int, default=64,
                        help="LSTM/attention hidden size (default 64 for CPU; 128+ for GPU)")
    ...
    print(f"  X_encoder: {X_enc.shape}  ({meta['input_length']} steps, {meta['n_enc_features']} features)")
    print(f"  X_decoder: {X_dec.shape}  ({meta['output_length']} steps, {meta['n_dec_features']} features)")
    print(f"  y_targets: {y.shape}")
    print(f"  Train/val: {len(train_idx):,} / {len(val_idx):,} samples")

    # ── DataLoaders
    train_ds = PREDISPATCHDataset(X_enc[train_idx], X_dec[train_idx], y[train_idx])
    val_ds   = PREDISPATCHDataset(X_enc[val_idx],   X_dec[val_idx],   y[val_idx])
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        nmape      = evaluate_nmape(model, val_loader, scalers)
        ...
        log_rows.append({
            ...
            "val_loss": round(val_loss, 6), "nmape": round(nmape, 4),
    ...
    print(f"  Best nMAPE (val): {best_row['nmape']:.2f}%")
