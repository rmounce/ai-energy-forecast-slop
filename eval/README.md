...
| `p5min_tactical` | No | Suspended, retained for reference | Explicit tactical-price revival work only |
...
Do not use `p5min_tactical`, `pd_direct`, `model_a_hybrid`, or
`lgbm_strategic` results as evidence about the APF extrapolation model unless
the experiment is explicitly a comparison against `amber_apf_lgbm`.
