...
| `script-sigenergy-ems.yaml` | HA script: sets SiG Energy EMS parameters (control mode, charge/discharge limits, SoC cutoffs, export limits). Writes the `desired_*` helpers + direct charge/discharge/PV registers, and calls `script.sigen_apply_limits` **synchronously** to push grid/PCS limits to hardware (deterministic Modbus ordering, no polling wait). On a **mode change** it lowers every limit to `min(current, target)` → apply → flip mode → raise to target → apply. The min-before-mode ordering protects the *instant* limits (import/PCS/charge/discharge) entering their active modes; the export burst is handled separately by `automation-sigenergy-export-ramp-hold.yaml` (the 300 W/s grid-limit ramp). When the mode is unchanged it writes targets and applies directly. |
| `script-sigenergy-apply-limits.yaml` | HA script (`script.sigen_apply_limits`): the **single source of truth** for writing the four Sigen grid/PCS `*_limitation` registers. Computes effective values — grid export `= min(desired_export, flexible_export(SAPN), amber_limit)`, grid import `= desired_import` (also clears the SAPN >4e6 import bug), pcs export `= min(desired_pcs_export, transient_pcs_export_cap(ramp-hold))`, pcs import `= desired_pcs_import` — and writes each only if it differs (idempotent). `mode: queued`. Called synchronously by the EMS script and by the master-limit automation. |
| `automation-sigenergy-master-limit.yaml` | HA automation: **external-event handler** only. Captures externally-imposed SAPN flexible export limits into `input_number.flexible_export_limit`, then delegates all enforcement to `script.sigen_apply_limits`. Fires on register changes (external/SAPN writes, import bug), `desired_*`/`transient_pcs_export_cap` helper changes (so manual edits and the ramp-hold propagate), Amber price changes, and a 5-min failsafe. No longer contains the write logic itself. |
| `automation-sigenergy-export-ramp-hold.yaml` | HA automation: works around the Sigen grid-export-limit register's internal **~300 W/s ramp**. On a **decrease** of `number.sigen_plant_grid_export_limitation` (from EMHASS desired, Amber negative-price, or SAPN flexible limit — `apply_limits` folds all into this register), it holds actual grid export at the new target by closed-loop capping the *instant* PCS export limit (`PCS_cap = p_pcs + p_grid + E_target`, event-driven on grid-power updates) via `input_number.transient_pcs_export_cap` for the modelled ramp duration, then releases. Writing a dedicated helper (which `apply_limits` mins with `desired_pcs_export_limit`) keeps it from fighting the EMS script. This is the actual fix for the transient over-export/burst when reducing export; up-ramps are tolerated. |

> **New helper required in HA:** `input_number.transient_pcs_export_cap` (min 0, max
> 100, step 0.001, **kW**, default 100). The export ramp-hold writes it; `apply_limits`
> mins it with `desired_pcs_export_limit`. If absent, `apply_limits` defaults it to 100
> (no transient cap) and degrades gracefully. `script.sigen_apply_limits` must be
> created with that object_id so `script.configure_sigen_ems_state` and the master-limit
> automation can call it.
