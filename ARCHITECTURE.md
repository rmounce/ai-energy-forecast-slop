...
| `script-sigenergy-ems.yaml` | HA script: sets SiG Energy EMS parameters (control mode, charge/discharge limits, SoC cutoffs, export limits) by writing to Sigenergy number/select entities. On a **mode change** it lowers every limit to `min(current, target)`, briefly waits (≤2 s) for the async grid/PCS limits and the mode flip to land, then raises limits to target. This gives deterministic Modbus ordering — it is **not** the burst fix (the burst is the 300 W/s grid-limit ramp, handled by `automation-sigenergy-export-ramp-hold.yaml`). When the mode is unchanged it writes targets directly. |
